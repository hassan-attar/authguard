import { BaseServiceError, ServiceError } from "./service-error";

/**
 * Represents a server error, typically a 500 Internal Server Error.
 * 
 ...
 * 
 * The `ServerError` class provides a default implementation for the `serializeErrors`
...
class ServerError extends ServiceError {
    ...
    constructor(logMessage: string, error: Error) {
        super(logMessage);
        Object.setPrototypeOf(this, ServerError.prototype); // Set prototype explicitly for inheritance in TypeScript
        this.error = error;
    }
